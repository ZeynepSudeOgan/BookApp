...
class DataManager {
  ...
  static Future<List<Book>> getRecommandations() async {
    var all = await getAllBooks();
    var randoms = _getRandomElements<Book>(all, all.length ~/ 2);
    return randoms;
  }
  ...
  static Future<List<Book>> getBooksByCategory(Category category) async {
    var books = await getAllBooks();
    return [books[0], books[2], books[5]];
  }

  static Future<List<Book>> getReadBooks(int userID) async {
    var books = await getAllBooks();
    return [books[1], books[4], books[2]];
  }

  static Future<List<Book>> getWillReadBooks(int userID) async {
    var books = await getAllBooks();
    return [books[2], books[3], books[5], books[1]];
  }

  static Future<List<Book>> getReadingBooks(int userID) async {
    var books = await getAllBooks();
